...
**Reason**: Family invite and household QR are owned by the **family list** (and related household flows), not profile details.

**Migration**: Users obtain scan-to-join and per-household QR from **Settings → Family** (family list). Profile details shows identity and account actions only.

### Requirement: Profile details explains family-invite use

**Reason**: Profile details no longer shows a family-invite QR; explanatory copy for invite belongs on family/household surfaces.

**Migration**: Show family-invite helper copy on family list or household QR share UI as needed.
